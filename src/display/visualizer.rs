...
use comfy_table::{Table, Attribute, Cell, ContentArrangement, Width, ColumnConstraint};
...
impl JsonVisualizer for TableVisualizer {
    fn visualize(&self, data: &[JsonValue], options: &DisplayOptions) -> String {
        ...
        let headers: Vec<Cell> = all_keys.iter()
            .map(|key| {
        ...
        for json_obj in &processed_data {
            let row: Vec<Cell> = all_keys.iter()
                .map(|key| {
    ...
    let text = match value {
        ...
        JsonValue::Array(arr) => {
            if arr.is_empty() {
                ...
                format!("[{}]", arr.iter()
                    .map(|v| format_cell_value_for_comfy_table(v, &options.clone().with_color(false)))
                    .collect::<Vec<_>>()
                    .join(", "))
            } else {
                format!("[{} items]", arr.len())
        ...
        JsonValue::Object(obj) => {
            if obj.is_empty() {
                ...
                format!("{{{}}}", obj.iter()
                    .map(|(k, v)| format!("{}: {}", k, format_cell_value_for_comfy_table(v, &options.clone().with_color(false))))
                    .collect::<Vec<_>>()
                    .join(", "))
            } else {
                format!("{{{} fields}}", obj.len())
