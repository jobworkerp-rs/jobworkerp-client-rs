...
pub fn job_result_to_json(
    ...
    if let Some(data) = &job_result.data {
        result_json["job_id"] = serde_json::json!(
            data.job_id.as_ref().map(|j| j.value).unwrap_or_default()
        );
        ...
        result_json["status"] = serde_json::json!(
            status_formatter.format(data.status(), format)
        );

        // Format timestamps
        result_json["start_time"] = serde_json::json!(
            format_timestamp(data.start_time, format)
        );
        result_json["end_time"] = serde_json::json!(
            format_timestamp(data.end_time, format)
        );
        result_json["duration"] = serde_json::json!(
            format_duration(data.start_time, data.end_time, format)
        );
        ...
        if let Some(output) = &data.output {
            if let Some(descriptor) = result_descriptor {
                ...
                match ProtobufDescriptor::get_message_from_bytes(descriptor, &output.items) {
                    Ok(message) => {
                        match ProtobufDescriptor::message_to_json_value(&message) {
                            Ok(output_json) => {
                                result_json["output"] = output_json;
                            }
                            Err(_) => {
                                result_json["output"] = serde_json::json!(
