...
use crate::jobworkerp::data::{Worker, QueueType, ResponseType};
...
pub fn worker_to_json(
    ...
    if let Some(data) = &worker.data {
        ...
        worker_json["runner_id"] = serde_json::json!(
            data.runner_id.as_ref().map(|r| r.value)
        );
        ...
        worker_json["queue_type"] = serde_json::json!(
            queue_type_formatter.format(data.queue_type(), format)
        );
        worker_json["response_type"] = serde_json::json!(
            response_type_formatter.format(data.response_type(), format)
        );

        // Format boolean values with decoration
        worker_json["store_success"] = serde_json::json!(
            format_boolean(data.store_success, format)
        );
        worker_json["store_failure"] = serde_json::json!(
            format_boolean(data.store_failure, format)
        );
        worker_json["use_static"] = serde_json::json!(
            format_boolean(data.use_static, format)
        );
        worker_json["broadcast_results"] = serde_json::json!(
            format_boolean(data.broadcast_results, format)
        );
            ...
            match ProtobufDescriptor::get_message_from_bytes(descriptor, &data.runner_settings) {
                Ok(settings_message) => {
                    match ProtobufDescriptor::message_to_json_value(&settings_message) {
                        ...
                        Err(_) => {
                            worker_json["runner_settings"] = serde_json::json!(
                                String::from_utf8_lossy(&data.runner_settings)
                            );
                        }
                    }
                }
...
mod tests {
    ...
    fn test_format_boolean() {
        // Test table format
        assert_eq!(
            format_boolean(true, &DisplayFormat::Table),
            "true"
        );
        
        // Test card format
        assert_eq!(
            format_boolean(false, &DisplayFormat::Card),
            "❌ false"
        );
        assert_eq!(
            format_boolean(true, &DisplayFormat::Card),
            "✅ true"
        );
        
        // Test JSON format
        assert_eq!(
            format_boolean(false, &DisplayFormat::Json),
            "false"
        );
    }
}