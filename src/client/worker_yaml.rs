...
/// fields (`store_success` / `store_failure` / `use_static` /
/// `broadcast_results`). Pasting them at a string-typed position
/// produces a type error at YAML→proto encoding time, which is a more
/// precise error than a blanket pre-parse rejection.
///
/// Operators whose values legitimately start with one of the
/// leading-position indicators must sanitize the value before
/// exporting it — wrapping the placeholder in YAML quotes does *not*
/// loosen this check, because the expander runs before YAML parsing
/// and has no way to see surrounding quotes.
fn check_yaml_safe_scalar(value: &str) -> std::result::Result<(), String> {
    if value.contains('\n') || value.contains('\r') {
    ...
    // Whole-value structural breakers. Two sub-classes:
    //
    // * Flow-collection terminators (`,` `]` `}`) split or close a flow
    //   collection if the host context is a flow collection — context
    //   we cannot detect from raw text.
    //
    // * Quote / escape characters (`"`, `'`, `\`). These are the
    //   precise characters that turn `field: "${VAR}"` (the documented
    //   "wrap in quotes" pattern) into a parse error when the
    //   substituted value contains them mid-string. e.g.
    //   `SECRET="abc\"def"` → after expansion `field: "abc"def"`,
    //   which terminates the quoted scalar early. Banning these
    //   characters anywhere in the value keeps the structure-safety
    //   guard's promise honest in both block and quoted contexts; the
    //   trade-off is that env values containing literal quotes or
    //   backslashes must be moved out of YAML (e.g. via `$file:`).
    ...
    // Leading-position indicators: only meaningful at the very start
    // of the scalar. Allowing them mid-string keeps DSN / URL / regex
    // values working (e.g. `postgres://...?a=1&b=2`).
    if let Some(first) = value.chars().next() {
        ...
        // Two-character leading indicators (only when followed by space).
    ...
    // Boolean / null literals (`true` / `false` / `yes` / `no` / `null` /
    // `~`) are intentionally NOT rejected here. They are required for
    // legitimate uses against bool fields like `store_failure`,
    // `use_static`, and `broadcast_results` (see WorkerSpec) — for
    // example `store_failure: ${STORE_FAILURE:-true}`. If such a value
    // ends up at a string-typed position in the document the type
    // mismatch surfaces during downstream YAML→proto encoding, which is
    // a more precise error than a blanket pre-parse rejection.
    Ok(())
}
...
";
    ...
    fn allows_env_value_that_is_boolean_literal() {
        ...
        let out = expand_env(raw).expect("bool literal must pass through");
    ...
    fn allows_env_value_that_is_null_literal() {
        ...
        let out = expand_env(raw).expect("null literal must pass through");
...
";
        let expanded = expand_env(yaml).expect("env expansion must succeed for bool defaults");
